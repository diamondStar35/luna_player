...
namespace PrismSharp.Speech.ScreenReaders
{
    internal sealed class Reader : IScreenReader
    {
        ...
        /// <inheritdoc />
        public event Action<SpeechBackendAvailability>? BackendAvailabilityChanged;
        ...
        public Features Capabilities
        {
            ...
            {
                lock (_sync)
                    return _backend?.Features ?? Features.None;
            }
        }
            ...
            lock (_sync)
            {
                ...
                try
                {
                    _context = new Context(new ContextOptions
                    {
                        ...
                        AutoPowerManage = Context.AutoPowerManagementSupported
                    });
        ...
        /// <summary>
        /// Invoked on Prism's availability poll thread. It deliberately takes no lock and touches no
        /// backend: the poll thread cannot scan again until this returns, and the backend is owned by
        /// the speech worker thread. Subscribers are responsible for marshalling their own work.
        /// </summary>
        private void OnAvailabilityChanged(ulong backendId, string name, bool available)
        {
            var handler = BackendAvailabilityChanged;
            if (handler == null)
                return;

            try
            {
                handler(new SpeechBackendAvailability(backendId, name, available));
            }
            catch
            {
            }
        ...
        // Enumeration deliberately creates without initializing: the feature bits, including runtime
        // availability, are valid on an uninitialized backend, and initializing every registered
        // backend merely to list them is both slow and side-effecting.
        private static bool TryProbeSupportedBackend(Context context, BackendInfo info, out string backendName)
        {
            ...
            try
            {
                using var backend = context.CreateUninitialized(info.Id);
                if (!backend.IsSupportedAtRuntime)
                    return false;
        ...
        private bool SpeakToPlayerLocked(string text, bool interrupt)
        {
            ...
            var firstChunk = true;
            foreach (var chunk in _backend.SpeakToMemory(text))
            {
                _player.Write(chunk.Samples.Span, chunk.Channels, chunk.SampleRate, interrupt && firstChunk);
                firstChunk = false;
            }
